...
### The `declare` Keyword (Class Fields)

Use `declare` inside a class to tell TypeScript that a field exists at runtime but should not be emitted or initialized in the compiled JavaScript. This is helpful when a property is set externally (frameworks, mixins, dependency injection) or later in the lifecycle, and you want to avoid definite-assignment errors without generating a class field.

- Eliminates the "Property has no initializer and is not definitely assigned" error.
- No JavaScript output for the field; the value comes from later assignment.
- Different from `!` (definite assignment assertion), which still emits the field.

```typescript
class Bird {
  // Provided later (e.g., hydration from API or framework)
  declare name: string
...
// Name arrives later (e.g., from server/state)
parrot.name = 'Polly'
parrot.describe() // Polly the Parrot
```
...
class Goldfish implements Pet {
  play() {
    console.log('Swimming around happily!')
  }
  feed() {
    console.log('Eating flakes...')
  }
}
```
...
Inheritance can lead to rigid hierarchies and the "fragile base class" problem. Composition offers more flexibility. Instead of inheriting from a `Veterinarian` class, a `Zoo` class should _have_ a `Veterinarian`.

```typescript
class Veterinarian {
  checkHealth(animal: string) {
    console.log(`Checking health of ${animal}`)
  }
}
...
Design patterns are typical solutions to common problems in software design. To learn more about how to apply design patterns in TypeScript with practical examples, see [Design Patterns in TypeScript](/my-learning-curve/blog/20-design-patterns).
