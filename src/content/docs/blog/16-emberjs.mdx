...
_Classic approach (Separate files):_
...
1. **Mutating State**: In React, we _never_ mutate state directly (`count++` is forbidden). In Ember with `@tracked`, **mutation is the standard way** to trigger updates (`this.count++` works perfectly).
