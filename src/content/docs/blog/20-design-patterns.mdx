...
These are aspect-oriented programming style and require decorators to be enabled in `tsconfig.json`. They are conceptually similar to the Decorator pattern, but implemented at the language level.
...
class EmailSubscriber implements Observer {
  update(data: any) { console.log(`Email subscriber received: ${data}`); }
}
