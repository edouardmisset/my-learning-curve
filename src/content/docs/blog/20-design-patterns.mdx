...
// Usage
const admin = new UserBuilder('Alice')
...
**Real-world use case:** Adding logging, caching, or validation to API handlers (common in NestJS or with TS [decorators](https://www.typescriptlang.org/docs/handbook/decorators.html#handbook-content)).
...
This follows the **aspect-oriented programming** style. The [TypeScript decorators handbook](https://www.typescriptlang.org/docs/handbook/decorators.html) covers the legacy (stage 2) decorator implementation, which requires enabling [`experimentalDecorators`](https://www.typescriptlang.org/tsconfig#experimentalDecorators). Standard (stage 3) decorators are supported since TypeScript 5.0 and use different signatures and semantics.

The example below uses the standard (stage 3) decorator shape described in the [TypeScript 5.0 release notes](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-5-0.html#decorators).
...
const processOrder = (amount: number, strategy: PaymentStrategy) =>
  strategy(amount)
...
class XMLExportVisitor implements Visitor {
  ...
  visitSquare(s: Square) {
    console.log('Exporting Square to XML')
  }
}
...
**Adapter**, **Facade**, and **Proxy** can look similar because they all wrap another object. The key difference is _why_ they wrap: compatibility, simplification, or access control.
...
**Strategy**, **State**, and **Command** can all move logic out of a big conditional. The key difference is _who_ decides the behavior: a caller, the object itself, or a queued request.
