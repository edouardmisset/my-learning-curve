...
While not native classes in JS (usually implemented with Arrays), these _patterns_
...
A specific type of tree where each node has _at most_ two children (left and
right). For any node, the left child is _smaller_ and the right child
is _larger_ (or equal, depending on how duplicates are handled).

**Use case**: Efficient searching and sorting — O(log n) for _balanced_ trees.
