...
- **AI** ([Artificial intelligence](https://en.wikipedia.org/wiki/Artificial_intelligence)) is the broad umbrella term. In product discourse, it usually means
  software using machine learning.
- **LLM** ([Large language model](https://en.wikipedia.org/wiki/Large_language_model)) means large language model: a model trained to predict tokens in
  sequence.
- **Tokens** are chunks of text models process, price, and remember. They are
  ...
  slightly petty.
- **Inference** ([Inference](https://en.wikipedia.org/wiki/Inference)) is the act of using a trained model to generate an output.
- **Training** ([Training](https://en.wikipedia.org/wiki/Machine_learning#Training)) is the expensive phase where the model learned from huge
  datasets.
- **Non-deterministic** ([Non-determinism](https://en.wikipedia.org/wiki/Nondeterministic_programming)) means the same input can produce slightly different
...
The **context window** ([Context window](https://en.wikipedia.org/wiki/Context_window)) is the maximum amount of text and multimodal input the
...
irrelevant stack traces. Many models also suffer from the "Lost in the Middle"
effect, where performance degrades as the relevant information moves farther
away from the edges of the context, even when the window is large.

**Multimodality** ([Multimodal learning](https://en.wikipedia.org/wiki/Multimodal_learning)) means the system can operate across more than text: images,
...
choice, and hardware requirements. In practice, local inference usually means a
quantized model, often 4-bit or 8-bit, which changes memory bandwidth demands
and can introduce different sampling behavior from FP16 or BF16 inference on a
large cluster. "Runs on my laptop" is a real feature. It is just not a free
one.
...
The line is getting blurrier, though, because many **agentic workflows** now
use LLMs to choose how to execute each step inside an otherwise predefined
supervision path.
...
`AGENTS.md`, tool manifests, or other skill and agent metadata files. These are
not universal standards, but they do capture a real shift: developers are
starting to package instructions, tool access, and behavior contracts as part
of the product surface. In engineering terms, those **skills** are usually a
mix of tool definitions, often expressed as JSON Schemas, plus system prompts
and a little workflow glue. The abstraction is useful, but it is still
structured data and instructions under the hood.
...
returns something parseable after a heartfelt plea in plain English, you give
it a schema. JSON mode and similar mechanisms reduce ambiguity and make the
system easier to automate. Structured Outputs go a step further by enforcing
schema adherence. Many systems also use **constrained decoding** or other token-
level restrictions to keep the output within a valid format. The model can
still fail, but at least it can fail in a format your code understands.
...
prompt, run, inspect, tweak, rerun. If you prefer **Ralph loop**, think of it as
the very human habit of poking the system, hoping the next pass behaves better.
Just do not mistake repeated prodding for architecture.
...
property you manage. In practice, the engineering metric you are trying to
improve is **grounding**: how well the model stays anchored to the provided
context and tool results. Retrieval helps. Tool use helps. Verification helps.
Lower temperature sometimes helps. None of these make the problem disappear.
...
That is not the same as open source, because the license may still impose
important limits. Proprietary models do not publish their weights. That affects
cost control, portability, privacy, local deployment, reproducibility, and how
much leverage a vendor has over your roadmap.
...
desired behavior. **Fine-tuning** ([Fine-tuning](<https://en.wikipedia.org/wiki/Fine-tuning_(machine_learning)>)) is the broader category: adapting a base model
on a task or domain. **RLHF**, ([Reinforcement learning from human feedback](https://en.wikipedia.org/wiki/Reinforcement_learning_from_human_feedback)), and
...
these terms interchangeably when it should not. SFT teaches by example; RLHF or
RLAIF ranks and refines behavior with preference signals; product prompting
still does a great deal of the everyday steering.

**Distillation** ([Knowledge distillation](https://en.wikipedia.org/wiki/Knowledge_distillation)) is the process of training a smaller model to imitate a stronger
...
Then we reach architecture jargon. **MoE**, ([Mixture of experts](https://en.wikipedia.org/wiki/Mixture_of_experts)), means the
...
alone can be misleading. Most modern MoE systems rely on sparse activation, so
only a fraction of the parameters are active per token. A model can be enormous
on paper and still use only part of itself per token.

**Quantization** ([Quantization](https://en.wikipedia.org/wiki/Large_language_model#Quantization)) reduces the numerical precision used to represent model weights
and activations. The point is practical: smaller memory footprint, cheaper
serving, faster local deployment, sometimes a slight quality hit. It also
reduces memory bandwidth pressure, which is often the real bottleneck because
weights have to move from VRAM to the compute units fast enough to keep them
busy. If you care about self-hosting, quantization stops being obscure
optimization trivia and becomes part of the shopping list.
...
Google DeepMind, xAI, and Mistral all operate first-party inference surfaces.
AWS Bedrock, Vertex AI, Azure AI Foundry, and similar platforms also expose models
...
Specifically, for developers, you need to distinguish between **TTFT (Time to First Token)**—the
latency of the initial response—and **TPS (Tokens Per Second)**, which measures
the sustained throughput of the generation. A model might have a fast TTFT but
low TPS, making it feel snappy initially but slow for long-form content. For
batch jobs, TPS is usually the more relevant number; for interactive chat, TTFT
drives the user's first impression.
...
materially. Under the hood, this usually depends on **KV caching**, while prompt
caching is the product-facing feature that takes advantage of it. If not, your
elegant orchestration may simply be a more expensive way to restate yourself.
...
The highest-risk version of this is **indirect prompt injection**, where the
malicious instruction arrives through retrieved documents, web pages, or other
untrusted context rather than directly from the user.
...
the verification bill arrives immediately. A task that takes 10 seconds to
generate but 5 minutes to verify can easily destroy the ROI unless verification
...
more objective than it is, which is arguably worse. They do not just inherit
bias; they can amplify it by compressing uneven human data into outputs that
sound like consensus.
...
- [OpenAI Developers, "Working with evals"](https://developers.openai.com/api/docs/guides/evals) [deprecated]
